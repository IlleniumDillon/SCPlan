...
void SCElementMap::subscribeToUpdateTopic()
{
  try {
...
void SCElementMap::unsubscribe()
{
  MFDClass::unsubscribe();
  unsubscribeToUpdateTopic();
}

void SCElementMap::unsubscribeToUpdateTopic()
{
  update_subscription_.reset();
}

void SCElementMap::updateAlpha()
{
...
void SCElementMap::updateDrawUnder() const
{
...
void SCElementMap::incomingUpdate(const map_msgs::msg::OccupancyGridUpdate::ConstSharedPtr update)
{
...
bool SCElementMap::updateDataOutOfBounds(
...
void SCElementMap::updateMapDataInMemory(
...
void SCElementMap::createSwatches()
{
...
void SCElementMap::doubleSwatchNumber(
...
void SCElementMap::tryCreateSwatches(
  size_t width,
  size_t height,
  float resolution,
  size_t swatch_width,
...
size_t SCElementMap::getEffectiveDimension(
...
void SCElementMap::showValidMap()
{
  size_t width = current_map_.info.width;
  size_t height = current_map_.info.height;
...
void SCElementMap::resetSwatchesIfNecessary(size_t width, size_t height, float resolution)
{
...
void SCElementMap::updateSwatches() const
{
  for (const auto & swatch : swatches_) {
...
void SCElementMap::updatePalette()
{
...
void SCElementMap::transformMap()
{
  if (!loaded_) {
    return;
  }
...
void SCElementMap::update(float wall_dt, float ros_dt)
{
  (void) wall_dt;
  (void) ros_dt;
...
void SCElementMap::onEnable()
{
  MFDClass::onEnable();
  setStatus(rviz_common::properties::StatusProperty::Warn, "Message", "No map received");
}

void SCElementMap::updateMapUpdateTopic()
{
  unsubscribeToUpdateTopic();
  reset();
  subscribeToUpdateTopic();
