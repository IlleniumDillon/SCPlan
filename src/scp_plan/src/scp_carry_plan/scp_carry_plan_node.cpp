...
SCPCarryPlanNode::SCPCarryPlanNode() : Node("scp_carry_plan_node")
{
    loadConfig();
    sub_model_state_list_ = this->create_subscription<scp_message::msg::ModelStateList>
        ("model_states", 1, std::bind(&SCPCarryPlanNode::modelStateListCallback, this, std::placeholders::_1));
    pub_grid_map_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>("grid_map", 1);
    timer_ = this->create_wall_timer(std::chrono::milliseconds(1000), std::bind(&SCPCarryPlanNode::timerCallback, this));

    grid_map_ = GridMap(-scene_width_ / 2, -scene_height_ / 2, scene_width_ / 2, scene_height_ / 2, position_resolution_, yaw_step_);
    carry_plan_.config(agent_v, agent_w, agent_dt);

    sub_goal_ = this->create_subscription<scp_message::msg::ScpCarryTask>
        ("nscp_carry_goal", 1, std::bind(&SCPCarryPlanNode::goalCallback, this, std::placeholders::_1));
    pub_path_ = this->create_publisher<nav_msgs::msg::Path>("path", 1);
    RCLCPP_INFO(this->get_logger(), "SCP Carry Plan Node has been started.");
}

void SCPCarryPlanNode::loadConfig()
{
...
void SCPCarryPlanNode::loadModelConfig(std::string model_name, std::vector<Point> *shape, std::vector<Point> *anchor)
{
...
void SCPCarryPlanNode::modelStateListCallback(const scp_message::msg::ModelStateList::SharedPtr msg)
{
